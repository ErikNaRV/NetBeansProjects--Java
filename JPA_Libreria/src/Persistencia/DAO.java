...
public class DAO<T> {
    ...
    // Este método nos permite buscar un registro de la base de datos.
    // Como parámetro se pasa el ID del objeto a buscar de la base de datos.
    // Se busca en la base de datos el registro que contenga la misma información
    // que el parámetro recibido, y se guarda en un objeto.
    public Autor buscarAutorPorId(Integer Id) {
        try {
            Autor A1 = new Autor();

            A1 = em.find(Autor.class, Id);

            return A1;

        } catch (Exception e) {

            System.out.println("El autor se encuentra relacionado con libros existentes ");
